...
export class WebSocketServer<Response extends WebsocketResponse> {
  ...
  async streamRoute(path: string, messageType: string, topic: Topic, options: {
    ...
    resolveUserChannel: (token: string) => Promise<string>
  }) {
    ...
    incomingStream.on('data', async event => {
      const { response, token: userToken } = WebSocketServer.deserializeStreamEventToWebsocketMessage(event);

      const subscriptionKey = await options.resolveUserChannel(userToken)
      if (!subscriptionKey || !this.server.numSubscribers(subscriptionKey)) {
        moduleLogger.warn({ event, token: subscriptionKey }, 'WS Response Precheck: No subscribers for messageSourceId');
        return;
      }

      const published = this.server.publish(subscriptionKey, JSON.stringify(response));

      if (!published) {
        moduleLogger.warn({ event }, 'WS Response Postcheck: No subscribers for token messageSourceId');

      } else {
        moduleLogger.info({ token: subscriptionKey }, 'WS Response: Published message to subscribers');
      }
    });
