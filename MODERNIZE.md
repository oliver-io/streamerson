...
>
> **Sequencing rationale:** Step 1 first because we can't safely refactor on top of docs/examples that lie — and walking the consumer path surfaces the real rough edges. Step 2 second because a stable, slim toolchain (one runtime, one package manager, no native build) makes everything after it cheaper. Only after both do the PROJECT.md gaps become the actual development work.

---

## Step 1 — Make the docs & examples current and verified

The repo's docs and examples have drifted onto slightly stale APIs, and the doc-embedding pipeline has quietly let mismatches through. Goal: **every README and quick-start runs verbatim in a fresh sandbox, and the doc generator is idempotent.**

### 1.1 Understand & harden the doc-embedding pipeline
`yarn docgen` (`tsx ./tools/embed -w --summary`) runs three enrichments over every `**/*.md`:
- **Code embedding** — content between `<!-- BEGIN-CODE: <relpath> -->` and `<!-- END-CODE: ... -->` is replaced with the referenced source file, fenced and linked.
...
### 1.2 Fix stale API references (the half-finished `consumer-group` → `consumer` move)
...
- [ ] `packages/consumer/test/{consumer-single,consumer-many,consumer-new-messages,consumer-existing-messages}.ts` and `bootstrap.test.ts` — update to the current API (ties to test wiring in Step 2 / PROJECT.md Gap K).
...
### 1.3 Make every embedded example actually run
- [ ] Type-check and **run** each `*.example.ts` and each app (`app-hello-world`, `app-basic-crud`, `app-websockets`, `producers/example.ts`) against a local Redis (`yarn start:redis`).
...
### 1.4 Regenerate from verified sources
- [ ] Run `yarn docgen` and confirm it is **idempotent** (re-running produces no diff). Commit the regenerated docs.

### 1.5 Fresh-consumer sandbox validation — *(later; spec only)*
...
**Step 1 done when:** every quick-start runs verbatim in a fresh sandbox against a local Redis; `docgen` is idempotent; no references to deleted symbols remain; examples are covered by a CI smoke test.
...
### 2.1 Adopt Bun as runtime + package manager
...
### 2.2 Redis client — prototype-first, decoupled from the rest of the port
...
- [ ] Add `MAXLEN` trimming in this code either way (PROJECT.md Gap B).

### 2.3 Replace uWebSockets.js in `gateway-wss` with `Bun.serve` (lowest-risk win)
...
**Step 2 done when:** `bun install` + build + `bun test` are green; `gateway-wss` has no native addon; the runtime/PM is Bun with exactly one package manager; examples run under Bun. (The core Redis-client swap is *not* a gate — it ships only if the 2.2 spike passes; otherwise the streaming datasource stays on node-redis under Bun.)
...
Facts behind Step 2; re-verify before acting, since Bun moves fast.
